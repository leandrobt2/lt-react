class ImageHelper {
    ...
    getRandonImages(limit, type) {
        let size = this.images.length;

        if (type != undefined) {
            size = this.images.filter(m => m.type == type).length;
        }

        if (limit > size){
            if (type != undefined)
                return this.images.filter(m => m.type == type);
            else
                return this.images;
        }
        else {
            ...
            if (type != undefined)
                tempArray = tempArray.filter(m => m.type == type);
